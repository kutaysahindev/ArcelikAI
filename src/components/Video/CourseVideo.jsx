...
import { closeVideoWindow, completeVideo, setSelectedVideo } from "../../redux/videoSlice";

const CourseVideo = () => {
  // const videoRef = useRef(null);
  const dispatch = useDispatch();
  const {selectedVideo, completion, lastCompleted} = useSelector((state) => state.video);
  const {
    videoRef,
    videoDetails,
    watchAgain,
  } = useVideoPlayer();
  const { videoDuration, currentTime, progress, isCompleted } =
    videoDetails;
  ...
  return (
    <div className="video-container" id={`${selectedVideo}-cont`}>
      ...
      {(selectedVideo<1 || selectedVideo>3) && <div className="video-not-found"><MdReportGmailerrorred size={40}/> <p>Video Not Found</p></div>}
      {/* <p>Current Time: {currentTime.toFixed(2)} seconds</p> */}
      <div className='btn-container'>
      <button className="btns previous" onClick={() => videoSelector(-1)} disabled={(selectedVideo <= 1)}>
        <FaBackward />
        <span>Previous</span>
      </button>
      <button className="btns again" onClick={watchAgain}>
        ...
        {selectedVideo < 3 ?
        (
          <button className="btns next" onClick={() => videoSelector(1)} disabled={(lastCompleted < selectedVideo)}>
            <span>Next</span>
            <FaForward />
          </button>
        ) : (
          <button className="btns close" onClick={() => dispatch(closeVideoWindow)} disabled={(lastCompleted < 3)}>
            Close
          </button>
        )
